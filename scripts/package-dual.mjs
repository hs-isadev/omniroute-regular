...
for(const [platform,label] of [['windows-x64','Windows'],['linux-x64','Linux']]){
  ...
  await cp(join(repo,'packages/zai-consumer-adapter'),join(payload,'app/packages/zai-consumer-adapter'),{recursive:true,force:true});
  ...
  await writeFile(join(target,'manifest.json'),JSON.stringify({version,platform,host:'antigravity',hosts:['opencode','antigravity','codex','claude-code','claude-web-consumer','glm-web-consumer'],files},null,2)+'\n');
...
await cp(join(repo,'skills'),join(release,'skills'),{recursive:true,dereference:true});
