...
await access(join(app,'skills/omniroute-first-delegation/SKILL.md'));
await access(join(app,'skills/tdd-workflow/SKILL.md'));
await access(join(app,'skills/focused-implementation/SKILL.md'));
await access(join(app,'skills/verify-change/SKILL.md'));
await assert.rejects(access(join(app,'skills/taste')), /ENOENT/);
await assert.rejects(access(join(app,'skills/skillopt')), /ENOENT/);
...
async function protocol() {
  ...
  try {
    ...
    const listing=await call('tools/list',{});assert.deepEqual(listing.tools.map(t=>t.name).sort(),['omni_models','omni_route','omni_routes','omni_task_approve','omni_task_cancel','omni_task_diff','omni_task_inspect_plan','omni_task_pause','omni_task_report','omni_task_resume','omni_task_status','omni_task_submit','omni_task_verify','omni_usage']);
