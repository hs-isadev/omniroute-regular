...
const release=resolve(import.meta.dirname,'../release/OmniRoute-Dual-0.5.1');
...
const archive=release+'.zip';try{await access(archive);throw new Error('Archive already exists; never silently overwrite a distributed release.');}catch(e){if(e.code!=='ENOENT')throw e;}
...
console.log(archive);console.log('SHA256 '+digest);console.log('Archive created; publication is a separate action.');
