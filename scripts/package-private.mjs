...
const repo=resolve(import.meta.dirname,'..'),version='0.6.1-private.1',release=join(repo,'release','OmniRoute-Private-'+version);
...
for(const [platform,label] of [['windows-x64','Windows'],['linux-x64','Linux']]){
  ...
  await writeFile(join(payload,'dual-provenance.json'),JSON.stringify({version,hosts,status:'private-family-package',personalDataIncluded:false,browserSessionsIncluded:false,publishable:false,sourceBaseline:'OmniRoute 0.6.1 private package with bounded API-worker swarms and one shared local browser consumer profile',notice:'Each recipient signs in with their own accounts. No CAPTCHA or anti-bot bypass is implemented.'},null,2)+'\n');
...
await writeFile(join(release,'PRIVATE-USE-NOTICE.txt'),'PRIVATE FAMILY PACKAGE — DO NOT PUBLISH ONLINE. No API keys, browser profiles, cookies, passwords, or account sessions are included. Setup opens one dedicated local browser profile with six user-controlled sign-in tabs. The adapters do not bypass CAPTCHA, anti-bot, rate-limit, or access controls.\n');
