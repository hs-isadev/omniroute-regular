...
for(const [platform,label] of [['windows-x64','Windows'],['linux-x64','Linux']]){
  ...
  await writeFile(join(payload,'dual-provenance.json'),JSON.stringify({version,hosts,status:'shareable-family-package',personalDataIncluded:false,browserSessionsIncluded:false,publishable:true,sourceBaseline:'OmniRoute 0.6.5: isolated provider health deadlines, metadata-only provider validation, verified bundled runtime paths, and registration-safe update/rollback',notice:'Each recipient signs in with their own accounts. No credentials or sessions are included, and no CAPTCHA or anti-bot bypass is implemented.'},null,2)+'\n');
