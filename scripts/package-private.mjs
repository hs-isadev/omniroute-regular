...
const repo=resolve(import.meta.dirname,'..'),version='0.6.2-private.1',release=join(repo,'release','OmniRoute-Private-'+version);
...
for(const [platform,label] of [['windows-x64','Windows'],['linux-x64','Linux']]){
  ...
  await writeFile(join(payload,'dual-provenance.json'),JSON.stringify({version,hosts,status:'shareable-family-package',personalDataIncluded:false,browserSessionsIncluded:false,publishable:true,sourceBaseline:'OmniRoute 0.6.2 family package with bounded API-worker swarms, guarded browser-consumer usage, and one shared local browser profile',notice:'Each recipient signs in with their own accounts. No credentials or sessions are included, and no CAPTCHA or anti-bot bypass is implemented.'},null,2)+'\n');
