...
async function routedFixture(failureCount=0){
  ...
  const models=config.providers.filter(p=>p.enabled).flatMap(p=>p.models.filter(m=>m.enabled&&m.allowed).map(m=>({providerId:p.id,modelId:m.modelId,enabled:true,allowed:true,health:{status:'healthy'},route:{freeStatus:'confirmed'},contextWindow:131072,maxOutputTokens:8192,reasoningEfforts:['none'],intelligenceTier:m.intelligenceTier,latencyTier:m.latencyTier,pricing:{inputPerMillionUsd:0,outputPerMillionUsd:0},capabilities:{text:true,coding:true,toolCalling:true}})));
  const calls=[];
