...
The host owns the canonical task, plan and acceptance criteria. This policy is model-neutral: work with whichever GPT-5.x/5.6 variant, Astra, or other host model is actually selected and available; do not assume a model name or claim that OmniRoute controls the host model. Delegate only independent work that saves effort. Before each delegation estimate packet input and expected output tokens, read trustworthy worker context metadata with omni_models, and reserve room for instructions, response and host synthesis. If limits are unknown or exceeded, shorten the packet or keep the work with the host. Use omni_route taskPacket for explicit bounded packets; send only relevant excerpts, interfaces, constraints, acceptance criteria and requested output, never whole transcripts. Keep browser consumers paced, small-only and outside parallel implementation swarms.
