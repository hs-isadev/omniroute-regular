...
Use the omniroute MCP server's omni_route tool first for nearly every substantive, ordinary, non-sensitive request: planning, repository reading, code/design reasoning, test interpretation, documentation, analysis, review, and suitable bounded implementation subtasks. Set routingMode="regular" so OmniRoute selects a free worker without an extra planner; let its intent classifier choose model strength. For substantive eligible tasks, obtain a worker response before doing extensive host reasoning or considering native delegation. Simple acknowledgments, approval decisions, status checks, latency-critical UI actions, and host-only mutations do not need a worker; for edits, delegate the bounded reasoning or review and keep the local mutation with the host.

The host owns the canonical task, plan and acceptance criteria. In Codex, prefer GPT-5.6 Sol for everyday orchestration, use GPT-6 Astra for unusually complex coordination, and use Terra or other suitable available free workers for clearly bounded execution; these are host preferences, not models OmniRoute can invoke or detect. Delegate only independent work that saves effort. Before each delegation estimate packet input and expected output tokens, read trustworthy worker context metadata with omni_models, and reserve room for instructions, response and host synthesis. If limits are unknown or exceeded, shorten the packet or keep the work with the host. Use omni_route taskPacket for explicit bounded packets; send only relevant excerpts, interfaces, constraints, acceptance criteria and requested output, never whole transcripts. Keep browser consumers paced, small-only and outside parallel implementation swarms.
...
function codexTomlBlock(nodePath: string, cliPath: string, runtimeRoot: string): string {
  return [
    ...
    'approval_mode = "approve"',
    "",
    "[mcp_servers.omniroute.tools.omni_task_submit]",
    'approval_mode = "approve"',
    "",
    "[mcp_servers.omniroute.tools.omni_task_status]",
    'approval_mode = "approve"',
    "",
    "[mcp_servers.omniroute.tools.omni_task_inspect_plan]",
    'approval_mode = "approve"',
    "",
    "[mcp_servers.omniroute.tools.omni_task_approve]",
    'approval_mode = "approve"',
    "",
    "[mcp_servers.omniroute.tools.omni_task_pause]",
    'approval_mode = "approve"',
    "",
    "[mcp_servers.omniroute.tools.omni_task_resume]",
    'approval_mode = "approve"',
    "",
    "[mcp_servers.omniroute.tools.omni_task_cancel]",
    'approval_mode = "approve"',
    "",
    "[mcp_servers.omniroute.tools.omni_task_diff]",
    'approval_mode = "approve"',
    "",
    "[mcp_servers.omniroute.tools.omni_task_verify]",
    'approval_mode = "approve"',
    "",
    "[mcp_servers.omniroute.tools.omni_task_report]",
    'approval_mode = "approve"',
  ].join("\n");
}
