...
async function enableHighThinking(target){
  const controls=target.locator('button:has-text("Extended thinking"),[role="button"]:has-text("Extended thinking"),button:has-text("Thinking"),[role="button"]:has-text("Thinking")');
  ...
  throw new Error('Claude high-thinking control is unavailable for this account or current model. OmniRoute should try another provider.');
}
...
process.stdin.on('data',chunk=>{
  ...
  for(;;){const newline=buffered.indexOf('\n');if(newline<0)break;const line=buffered.slice(0,newline).trim();buffered=buffered.slice(newline+1);if(!line)continue;
    void (async()=>{try{const message=JSON.parse(line);if(message.id===undefined)return;
      ...
      else if(message.method==='tools/list')send(message.id,{result:{tools:[{name:'claude_query',description:'Ask Claude through the signed-in dedicated browser.',inputSchema:{type:'object',properties:{prompt:{type:'string',minLength:1},highThinking:{type:'boolean',description:'Enable Claude high thinking before submitting.'}},required:['prompt'],additionalProperties:false}},{name:'test_connection',description:'Check the dedicated Claude browser login.',inputSchema:{type:'object',properties:{},additionalProperties:false}}]}});
