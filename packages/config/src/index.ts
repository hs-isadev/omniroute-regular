...
function mergeConfig(base: OmniConfig, input: unknown): OmniConfig {
  ...
  if (source.providers) {
    ...
    merged.providers = merged.providers.map((provider) => {
      ...
      let models = existing?.models ? [...existing.models] : [...provider.models];
      ...
      const result = { ...provider, ...(existing ?? {}), models };
      if (provider.id === "groq" && existing?.baseUrl === provider.baseUrl) {
        models = models.filter((model) => model.modelId !== "qwen/qwen3.6-27b");
        result.models = models;
        result.freeModelOrder = [...new Set((result.freeModelOrder ?? []).map((modelId) => modelId === "qwen/qwen3.6-27b" ? "qwen/qwen3.8-27b" : modelId))];
      }
      return result;
    });
